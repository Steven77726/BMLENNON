...
document.addEventListener("visibilitychange", applyInviteCopy);
...
const FORMSPREE_ENDPOINT = "https://formspree.io/f/meedldgz";
const CONFIRMATION_RECIPIENTS = "ghighielsa@gmail.com, stevenohayon@icloud.com";

function applyInviteCopy() {
  const inviteCopy = document.querySelector(".invite-copy");
  if (inviteCopy) {
    inviteCopy.innerHTML = INVITE_COPY_HTML;
  }
}
...
form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    const message = [
      ...
      "",
      `Destinataires souhaités : ${CONFIRMATION_RECIPIENTS}`
    ].join("\n");
    ...
    const response = await fetch(FORMSPREE_ENDPOINT, {
      ...
      body: JSON.stringify({
        email: "ghighielsa@gmail.com",
        subject: "Nouvelle confirmation — Bar Mitsvah Lennon",
        message,
        nom: payload.nom,
        prenom: payload.prenom,
        personnes: payload.personnes,
        telephone: payload.telephone,
        recipients: CONFIRMATION_RECIPIENTS
      })
    });
...
function downloadIcs() {
  const ics = [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//Lennon//Bar Mitsvah Invitation//FR",
    "CALSCALE:GREGORIAN",
    "METHOD:PUBLISH",
    "BEGIN:VEVENT",
    `UID:${Date.now()}@lennon-bar-mitsvah`,
  ...
  link.download = "bar-mitsvah-lennon.ics";
